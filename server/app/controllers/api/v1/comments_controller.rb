  ...
  module V1
    class CommentsController < ApplicationController
      ...
      def index
        ...
        total_page = feed.messages.page(@page).per(PAGE_PAR_NUM).total_pages
        @page = total_page
        messages =.feed.messages.page(@page).per(PAGE_PAR_NUM)

        render json: { status: 'success', data: { university: feed.university,
                                                  feed: feed,
                                                  messsages: messages,
                                                  pageNum: @page,
      ...
      def comment_params
        @comment_params ||= params.permit(:university_id, :feed_id, :name, :title, :content)
      end
